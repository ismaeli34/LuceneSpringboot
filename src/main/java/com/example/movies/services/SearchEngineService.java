...
//					indexWriter.close();
	...
	public SearchMovieResponse search(String srchTxt) {
		...
		try {
			...
			for (ScoreDoc scoreDoc : topDocs.scoreDocs) {
				...
				for (String cast : castList) {
					...
					String castObj[] = cast.split(",");
					if(castObj.length==3) {
						log.info("Cast size"+castObj.length + "movie id"+ movie.getId());
						movie.getCast().add(new Cast(castObj[0], castObj[1], castObj[2]));
					}
		...
		String duration = format.format((endTime-startTime)/1000.0);
		 
		    
		    SearchMovieResponse searchResponse = new SearchMovieResponse();
		    searchResponse.setDuration(duration);
		    searchResponse.setResult(movies);
